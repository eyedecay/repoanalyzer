...
const ChatBox = ({onSend}: ChatBoxProps) => {
    ...
    return (
        <div className="flex items-end gap-2 w-full max-w-2xl p-2 border border-base-content/20 rounded-xl focus-within:border-primary focus-within:ring-1 focus-within:ring-primary bg-base-100">
            <textarea className="w-full flex-1 min-h-[48px] p-2 bg-transparent resize-y outline-none focus:outline-none" placeholder="Ask Anything..." onChange = {(e) => setMessage(e.target.value)} value = {message} 
            onKeyDown={(e) => {
